...
public class AdminListCmd extends PrivateMineSubCommand {
   public AdminListCmd(PrivateMineCommand command) {
      super(command, "adminlist", "alist");
   }
   ...
   public boolean execute(CommandSender sender, List<String> args) {
      if (!args.isEmpty()) {
         ...
         this.getMinesManager().showPrivateMineAdminList(p);
         return true;
      } else {
   ...
   public String getUsage() {
      return "&e⚠ [&6XPrivateMines&e] &c/pmine adminlist &7~ &fOpens Private Mines GUI owned by players";
   }
