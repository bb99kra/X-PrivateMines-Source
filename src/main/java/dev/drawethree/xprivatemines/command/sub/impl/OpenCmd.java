...
public class OpenCmd extends PrivateMineSubCommand {
   public OpenCmd(PrivateMineCommand command) {
      super(command, "open");
   }
   ...
   public boolean execute(CommandSender sender, List<String> args) {
      if (!args.isEmpty()) {
         ...
         if (mine == null) {
            ...
            PlayerUtils.sendMessage(p, this.getMessageConfig().getMessage("mine-already-open"));
            return true;
         } else {
            this.getMinesManager().getAccessService().openPrivateMine(mine);
            PlayerUtils.sendMessage(p, this.getMessageConfig().getMessage("mine-open"));
            return true;
         }
      } else {
   ...
   public String getUsage() {
      return "&e⚠ [&6XPrivateMines&e] &c/pmine open &7~ &fOpen your mine to public";
   }
